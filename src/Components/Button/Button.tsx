...
const buttonClasses: { [k: string]: string } = {
    ...
    buttonClose: 'absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white',
}

export default function Button(props: PropTypes) {
    const svgName: string = props.svg
        ? capitaliseFirstLetter(props.svg)
        : ''

    const buttonClassNames = buttonClasses[String(props.type) + svgName]
        ? buttonClasses[String(props.type) + svgName]
        : ''
